      ...
      self.each(function () {
        var $el = $(this),
          countTo = $el.attr("data-count")*1;
                  $({someValue: 0}).animate({someValue: countTo}, {
                    duration: 2000,
                    easing:'swing',
                    step: function() {
    ...
    initScrollAnimation: function(){
      ...
      function animate(){
      		...
      		animateBlock.each(function () {
      		  var offset = $(this).offset().top;
      		   if ($(this).hasClass("ab--b")) {
      		    if (offset <= winScroll + animOffset || $(this).hasClass('instant')) {
        ...
        $(window).scroll(function(){
          ...
          var bottom = bit.winScroll + bit.winHeight + $('.main-footer').height();
          
          if( bottom + 20 > $(document).height()){
            var offsetBottom = bottom - $(document).height();
            $('.main_nav_js, .online-block').css('transform', 'translateY('+-offsetBottom+'px)');
          } else {
            $('.main_nav_js, .online-block').css('transform', 'translateY(0px)');
          }
        ...
        $(window).on('scroll', function() {
            $('.target').each(function() {
                if($(window).scrollTop() >= $(this).offset().top - 62 && $(window).scrollTop() < $(this).offset().top + $(this).height()) {
                    $('.main-nav a').removeClass('active');
                    $('.main-nav a[data-target="'+ $(this).attr('id') +'"]').addClass('active');
                }
            });
        });
